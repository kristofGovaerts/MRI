...
#Globals
FUNC_EXT='_basic_Full_V1.nii'
FUNC2_EXT = '_basic_Full_V5.nii'
...
reg_resample = 'flirt -applyxfm'

#functions
def RegAndResample(flt, atl, mask, mode='A'):
    ...
    out = flt.split('.')[0] + mode + '.nii.gz'
    if mode == 'A': #affine    
        args = '-in %s -ref %s -out %s -omat %s -dof 9 -nosearch' %(flt, atl, out, omat)         
    elif mode == 'R': #rigid
        args = '-in %s -ref %s -out %s -omat %s -dof 6 -nosearch -inweight %s' %(flt, atl, out, omat, mask)   
...
def brain_extraction(inimg, outimg):
    '''Uses FSL's BET to extract the brain - useful for registration. 
    Brain extraction is not great but good enough.'''
    
    args = '%s %s -n -m -f 0.5l -g 0.1' %(inimg, outimg)         
        
    bet = ['bet'] + args.split()  
    bet = ' '.join(bet)
    process = subprocess.call(bet, shell=True)
...
def apply_transform(t, ref, flo, filename):
    ...
    args = '-ref %s -in %s -init %s -out %s' %(ref, flo, t, filename)     
...
print "STEP 1: Affine reg of functional image to anatomical image."
for n, f in enumerate(filelist):
    ...
    print "Extracting brain from FUNC2."
    mask = brain_extraction(f2, f2.replace('.nii', '_brain.nii.gz')).replace('.nii.gz', '_mask.nii.gz')
    aff, mat = RegAndResample(f,a, mask, mode='R')
    aff2 = apply_transform(mat, a, f2, f2.replace('.nii', 'R.nii.gz'))
    
    #resave all images with proper resolution
    
    resave_with_affine(aff, orig_affine, aff.replace('.gz', '')) 
    resave_with_affine(aff2, orig_affine, aff2.replace('.gz', ''))     
    
    fl_a.append(a)
    fl_b.append(aff.replace('.gz', ''))
    fl_c.append(aff2.replace('.gz', ''))
    os.remove(a)
